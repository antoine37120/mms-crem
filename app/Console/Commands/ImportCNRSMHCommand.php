...
class ImportCNRSMHCommand extends Command
{
    /**
     * The name and signature of the console command.
     */
    protected $signature = 'import:cnrsmh
    ...
    public function handle()
    {
        ...
        $csvFile = $this->argument('csv') ?: storage_path('app/cnrsmh_matches.csv');
    ...
    private function processRow(array $row, int $userId, bool $isDryRun, array &$stats, int $lineNumber): void
    {
        try {
            ...
            $collection = $this->createOrGetCollection($collectionCode, $corpus, $year, $firstThreeDigits, $userId, $isDryRun, $stats);
     ...
     */
    private function createOrGetCollection(string $code, $corpus, string $year, string $firstDigits, int $userId, bool $isDryRun, array &$stats)
    {
        if ($isDryRun) {
    ...
    private function createOrGetItem($collection, string $collectionCode, string $filePath, array $fileInfo, int $userId, bool $isDryRun, array &$stats, int $lineNumber): void
    {
        ...
        try {
            ...
            if ($conflictingItem) {
                ...
                $stats['items_existing']++;
                $this->warn("  [CONFLIT] Item avec code '{$itemCode}' et extension '{$fileInfo['file_extension']}' existe déjà");
                return;
            }
    ...
    private function handleDuplicateError(string $itemCode, string $extension, string $newFilePath, int $lineNumber, array &$stats): void
    {
        ...
        if ($existingItem) {
            ...
            $this->warn("  [DOUBLON DÉTECTÉ] Item '{$itemCode}.{$extension}' existe déjà (ligne {$lineNumber})");
            $stats['items_existing']++;
        } else {
    ...
    {
        $conflictFile = storage_path('app/cnrsmh_conflicts.csv');
